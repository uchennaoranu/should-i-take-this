...
function evaluateDecision(a) {
  ...
  if (a.lockin === "yes" || a.exit === "no" || a.dependency === "yes") return { title: "DO NOT TAKE", summary: "The structure is broken. Do not proceed.", meaning: "This is not a pricing issue or a confidence issue. The structure itself creates direct risk through lock-in, weak exit, or dependency.", reasons, statusClass: "result-donot" };
  const conditionalFlags = [a.lockin === "unsure", a.exit === "partial", a.dependency === "partial", a.asymmetry !== "no", a.obligation !== "no", a.reversibility !== "yes"];
  if (conditionalFlags.some(Boolean)) {
    ...
    return { title: "CONDITIONAL", summary: "This only works if key terms are clarified. Do not proceed as is.", meaning: "The structure is not clearly broken, but it is not clean. Unclear terms, asymmetry, or weak reversibility need tightening before any commitment.", reasons, statusClass: "result-conditional" };
  }
